...
def parse_request(request, switch):
    ...
    ENABLE/switch/port: activate the specific port. This does not check whether you have multiple ports enabled simultaneously.
    ...
    TEST: returns a string to client 
    ...
    try:
        ...
        elif "test".find(cmds[0]) == 0:
            return switch.test_server()
        else:
...
def serve_requests():
    ...
    # Finds the address to bind.
    # Trying to setup pi to receive requests from lan, as opposed to internet address = netifaces.ifaddresses('wlan0')[2][0]['addr']
    #address = netifaces.ifaddresses('eth0')[17][0]['addr']
    # Creates an endpoint for accepting connections and binds it to the socket.
    #socket.bind("tcp://{}:5000".format(address))
    socket.bind("tcp://10.22.197.98:5000")
